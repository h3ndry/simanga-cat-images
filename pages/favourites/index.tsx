...
const Favourites: NextPage<{ m_data: any }> = ({ m_data }) => {
    const { breeds, setBreeds, favourites, setFavourite } = useContext(AppContext);

    useEffect(() => {
       setBreeds(m_data) 
        setFavourite(breeds.filter((item:any) => (item.isFavourite === true)))
